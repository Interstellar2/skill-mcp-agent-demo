...
async def _request_human_approval(prompt: str, arguments: dict) -> HumanApprovalRecord:
    """在 CLI 模式下请求人工确认.

    支持输入:
    - y / yes / 回车 → 批准
    - n / no → 拒绝（终止执行）
    - modify key=value → 修改参数后继续
    """
...
async def run_hitl_mode(
    ...
    HITL 配置来源（优先级从高到低）：
    1. Skill 的 metadata 中 human_in_the_loop 字段
    2. 环境变量 HITL_ALL_STEPS=1（所有步骤都确认）
    """
    async with SkillExecutorContext(
        ...
        async with get_mcp_tools() as (tools, session):
            ...
            async with RunTracker(
                skill_name, mode="hitl", variables=merged_vars
            ) as tracker:
                ...
                for idx, step in enumerate(steps, 1):
                    ...
                    if approval_prompt:
                        ha = await _request_human_approval(approval_prompt, arguments)
                ...
                logger.info("=" * 60)
                logger.info(f"🎉 HITL 执行完毕！{skill_name} 已完成~")
                logger.info("=" * 60)
